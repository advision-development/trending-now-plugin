...
`ADVTN_Path_Match::normalize()` reduces both sides of the comparison to one leading slash, no trailing slash, lowercase, no query string or fragment, repeated slashes collapsed, percent-encoding decoded and surrounding whitespace stripped *after* decoding — an encoded trailing space must not survive to defeat the trailing-slash rule. An entry carrying an explicit `scheme://` keeps only its path component, so a pasted `https://example.com/archive` means `/archive` instead of matching nothing at all; a URL with no path means `/`. Protocol-relative input is deliberately excluded from that rule — `//example.com/archive` stays the path `/example.com/archive`, because distinguishing a host from a doubled slash requires guessing and a doubled slash is the likelier typo in a list of paths.
...
The block's editor preview bypasses the gate: `wp-server-side-render` calls the render callback over REST carrying the *editor's* URL, not the reader's, so gating there would render the block blank while it is being edited. `is_admin()` separately covers admin-rendered contexts, such as the Widgets screen, where `REST_REQUEST` is never set.

Neither flag identifies an editor on its own, so the bypass requires `current_user_can( 'edit_posts' )` alongside **both**: `( ( defined( 'REST_REQUEST' ) && REST_REQUEST ) || is_admin() ) && current_user_can( 'edit_posts' )`. `REST_REQUEST` stays true for the life of any `/wp-json/*` request — including an anonymous, publicly readable `GET /wp/v2/pages/<id>?context=view`, which returns the block already rendered inside `content.rendered`. `is_admin()` has the same hole: `WP_ADMIN` is defined for `wp-admin/admin-ajax.php`, which dispatches `wp_ajax_nopriv_*` actions to logged-out visitors, so any theme rendering post content over admin-ajax — infinite scroll, "load more", AJAX page transitions — would otherwise bypass the gate for the public. The check costs a genuine preview nothing: the block-renderer endpoint the editor calls already requires edit permission in its own permission callback, and nothing in core renders this block under `is_admin()` for a user who cannot `edit_posts`, since the block widgets screen and the site editor both preview over REST. The two cheap flag checks come first so a front-end pageview never reaches `current_user_can()` and never loads a user to discover it is not editing. Accepted limitation: a logged-in editor browsing the public REST API still bypasses the gate, for content they can already edit.
